...
const connect = async ({ state, chainId, qrCodeModal, projectId, methods, events }: ConnectParams) => {
...
const WalletConnect: WalletBehaviourFactory<BridgeWallet, { params: WalletConnectExtraOptions }> = async ({
  id,
  options,
  store,
  params,
  ...
  const requestSignMessage = async (messageParams: SignMessageParams & { accountId?: string }) => {
  ...
  const requestSignTransactions = async (transactions: Array<Transaction>) => {
    ...
    const [block, accounts] = await Promise.all([provider.block({ finality: "final" }), requestAccounts()]);
  ...
  const requestSignIn = async (permission: nearAPI.transactions.FunctionCallPermission) => {
